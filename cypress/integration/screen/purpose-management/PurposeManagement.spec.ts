...
describe('Purpose Management Integration test', () => {
  ...
  it('should able to type purpose', () => {
    cy.get('.ant-modal-body')
      .get('#name').type('Purpose_Test')
      .get('.ant-select').find('#policyId').click().type('{enter}', {force: true})
      .get('#expireValue').type('1')
      .get('.ant-select').contains('DAY').click().type('{downarrow}{downarrow}{downarrow}{downarrow}{enter}')
      .get('#description').type('Purpose_Des')
      .get('.ant-btn')
      .contains('Create')
      .click()
  })
  ...
  it('should click row table', () => {
    cy.get('.ant-table-tbody').wait(3000)
      .find('td').contains('Purpose_Test')
      .click({ multiple: true })
  })
  ...
  it('should able to type edit purpose', () => {
    cy.get('#name').type(' Name')
      .get('#expireValue').type('{backspace}2')
      .get('.ant-select').click().type('{downarrow}{downarrow}{enter}')
      .get('#description').type(' Test')
      .get('.ant-transfer-list-content-item').find('input[type="checkbox"]').check().should('be.checked')
      .get('.anticon-right').click()
      .get('.ant-btn').contains('Save Draft').click()
      .get('.ant-btn').contains('OK').click()
  })

  it('should able to click Publish', () => {
    cy.get('.ant-btn').contains('Publish').click()
      .get('.ant-btn').contains('OK').click()
  })
