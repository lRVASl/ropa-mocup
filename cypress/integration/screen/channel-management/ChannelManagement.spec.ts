...
describe('Channel Management Integration test', () => {
  ...
  it('should able to remove channel table when length more than 1', () => {
    cy.get('#channelTable')
      .find('tbody tr').each(($tr, index) => {
        if (index >= 0) {
          if ($tr.text() === 'No Data') {
            return false
          } else {
            return cy.get('#channelTable')
  ...
  it('should able to type channel', () => {
    cy.get('.ant-modal-body')
      .get('[type="radio"]').check('Api')
      .get('#channelName').type('Channel Test')
      .get('#description').type('Channel Test_Description')
      .get('.ant-btn')
      .contains('Create')
      .click()
  })
  ...
  it('should able to type edit api endpoint', () => {
    cy.get('#channelName').type(' Api')
      .get('#description').type(' Api')
      .get('.ant-btn')
      .contains('Save')
      .click()
  })
  ...
  it('should able to add Purpose Profile', () => {
    cy.get('.ant-select').find('.ant-select-selector').click().type('{downarrow}{enter}')
  })
  ...
  it('should able to type extra field', () => {
    cy.get('.ant-modal-body')
      .get('#policyExtraFieldName').type('Extra Field_Test')
      .get('.ant-select').find('#policyExtraFieldType').click().type('{enter}', {force: true})
      .get('#policyExtraFieldDescription').type('Extra Field_Des')
      .get('#default').type('Test')
      .get('#isRequired').click()
      .get('.ant-btn')
      .contains('Create')
      .click()
  })
  ...
  it('should able to type edit extra field', () => {
    cy.get('.ant-modal-body')
      .get('#policyExtraFieldName').type('{backspace}{backspace}{backspace}{backspace}Edit')
      .get('.ant-select').find('#policyExtraFieldType').click({force: true}).type('{downarrow}{enter}', {force: true})
      .get('#policyExtraFieldDescription').type('{backspace}{backspace}{backspace}Edit')
      .get('#default').type('{backspace}{backspace}{backspace}{backspace}Edit')
      .get('#isRequired').click()
      .get('.ant-btn')
      .contains('Update')
      .click()
